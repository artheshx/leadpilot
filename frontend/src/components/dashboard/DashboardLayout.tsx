...
import {
  LayoutDashboard, Megaphone, FileText, Image, Users,
  CreditCard, LifeBuoy, Settings, LogOut, Zap, Menu, X,
  ChevronRight, Shield
} from 'lucide-react'
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/5">
        ...
        <span className="font-semibold text-white">Lead<span className="text-brand-400">Pilot</span></span>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-6 overflow-y-auto">
        {navItems.map(({ section, items }) => (
          <div key={section}>
            <p className="text-xs font-semibold text-slate-600 uppercase tracking-wider px-3 mb-2">{section}</p>
                ...
                <li key={href}>
                  <NavLink
                    ...
                    className={({ isActive }) => cn(
                      ...
                      isActive
                        ? 'bg-brand-600/15 text-white border border-brand-600/20'
      ...
      <div className="border-t border-white/5 p-3">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-all group">
          <div className="w-8 h-8 rounded-full bg-brand-700 flex items-center justify-center text-brand-200 text-xs font-bold flex-shrink-0">
    ...
    <div className="min-h-screen bg-dark-950 flex">
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-20 flex items-center gap-4 px-4 sm:px-6 h-14 border-b border-white/5 bg-dark-950/90 backdrop-blur-xl">
          ...
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-sm text-slate-500 flex-1">
            <span>LeadPilot</span>
      ...
      {/* AI Manager — available on every dashboard page; the only AI entrypoint the frontend uses */}
      <AiManagerWidget />
    </div>
  )
}
