...
them needed to change — the provider is chosen once, here.
    ...
    LLM_PROVIDER=grok       — needs GROK_API_KEY (xAI, OpenAI-compatible API)
    LLM_PROVIDER=groq       — needs GROQ_API_KEY (free tier, OpenAI-compatible API)
...
def chat(prompt: str, system: str = None, json_mode: bool = False, temperature: float = 0.4) -> str:
    ...
    if provider == "grok":
        return _grok_chat(prompt, system, json_mode, temperature)
    if provider == "groq":
        return _groq_chat(prompt, system, json_mode, temperature)
    raise LLMError(f"Unknown LLM_PROVIDER '{provider}'. Use 'ollama', 'anthropic', 'openai', 'grok', or 'groq'.")
...
# --------------------------------------------------------------------------
# Grok / xAI (hosted; LLM_PROVIDER=grok, needs GROK_API_KEY)
# API is OpenAI-compatible (chat/completions shape), just a different host.
# --------------------------------------------------------------------------

def _grok_chat(prompt: str, system: str, json_mode: bool, temperature: float) -> str:
    if not config.GROK_API_KEY:
        raise LLMError("LLM_PROVIDER=grok but GROK_API_KEY is not set. See backend/.env.example.")
    ...
    try:
        ...
        detail = getattr(e.response, "text", "") if getattr(e, "response", None) is not None else ""
        raise LLMError(f"Grok API request failed: {e} {detail}".strip()) from e

    data = resp.json()
    try:
        return data["choices"][0]["message"]["content"] or ""
    except (KeyError, IndexError) as e:
        raise LLMError(f"Grok returned an unexpected response shape: {data}") from e


# --------------------------------------------------------------------------
# Groq (hosted; LLM_PROVIDER=groq, needs GROQ_API_KEY) — free-tier friendly.
# Also OpenAI-compatible (chat/completions shape), different host again.
# --------------------------------------------------------------------------
