...
# Want $0 cost but hosted (no server to run)? Use "groq" — free tier, no card.
LLM_PROVIDER = os.getenv("LLM_PROVIDER", "ollama")  # "ollama" | "anthropic" | "openai" | "grok" | "groq"
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY", "")
ANTHROPIC_MODEL = os.getenv("ANTHROPIC_MODEL", "claude-sonnet-4-6")
OPENAI_CHAT_MODEL = os.getenv("OPENAI_CHAT_MODEL", "gpt-4o-mini")
# xAI's Grok — API is OpenAI-compatible, just a different base URL + key.
# Get a key at https://console.x.ai (Team Settings > API Keys).
GROK_API_KEY = os.getenv("GROK_API_KEY", "")
GROK_MODEL = os.getenv("GROK_MODEL", "grok-4")
GROK_BASE_URL = os.getenv("GROK_BASE_URL", "https://api.x.ai/v1")
# Groq (NOT the same company as Grok/xAI) — free-tier hosted inference,
# OpenAI-compatible API. Get a key at https://console.groq.com/keys (free,
# no card required). Good default if you want zero LLM cost to start.
